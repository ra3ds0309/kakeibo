...
- **設定画面**：右上のアカウント名をタップ →「設定」で開けます。機能のON/OFFとリリースノートを確認できます
- **毎月表示リセット**（設定でON、口座ごとに「口座の管理」から個別にON/OFF）：実際の残高（Firestore上の計算結果）は変えず、表示だけを直近の記録時点からの差分にできます。「今月分を記録してリセット」を押すと、その時点の残高を `resetBaseline` として保存し、以降はそこからの増減だけが表示されます。過去の記録は口座ドキュメントの `resetHistory` 配列に残ります
- **毎月の上限（クレジット）**（設定でON、口座ごとに金額を設定）：その口座の今月の支出合計と上限を比較したバーを表示します。超えても入力は制限されず、赤字表示になるだけです
- **チュートリアル**：`users/{uid}` ドキュメントの `tutorialSeenVersion` と、`src/utils/tutorial.js` の `CURRENT_TUTORIAL_VERSION` を比較して、古い場合に表示します。この定数を上げれば、既存ユーザーにも次回ログイン時に一度だけ再表示できます（リリースノートの追加もこのファイルで行います）
- **口座・カテゴリの自動作成を廃止**：新規ユーザーは口座・カテゴリ0件の状態から始まり、初回チュートリアルの中で最初の1つを作る流れになりました
